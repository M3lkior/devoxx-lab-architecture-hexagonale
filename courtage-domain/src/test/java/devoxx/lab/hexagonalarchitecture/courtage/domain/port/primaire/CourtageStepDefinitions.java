...
import devoxx.lab.hexagonalarchitecture.courtage.domain.port.secondaire.PortefeuilleRepositoryAdapter;
...
public class CourtageStepDefinitions implements Fr {
    ...
    public CourtageStepDefinitions() {
        ...
        Quand("on demande le calcul de la valeur du portefeuille {string}", (String idPortefeuille) -> {
            try {
                valeurPortefeuille = courtageService.recupererValeur(idPortefeuille);
            }catch (PortefeuilleNonGereException p){
                thrownException = p;
            }
        });
